...
import static com.zbc.constants.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        // 获取请求参数
        if (ContentType.JSON.getValue().equals(contentType)) {  // Post 请求
        ...
        if (ObjUtil.isNotNull(id)) {
            // 获取到请求路径的业务前缀: /api/space?xxx=aaa, 将 "/api/" 整体替换为空
            String requestURI = request.getRequestURI();
            // 此时剩下的部分就是业务, 关键
            String prtURI = requestURI.replaceAll(contextPath + "/", "");
            // 获取前缀的第一个斜杠前的字符串
            String moduleName = StrUtil.subBefore(prtURI, "/", false);
            // 大功告成
            switch (moduleName) {
                case "space":
